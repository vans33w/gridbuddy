...
export default function RacesPage() {
  ...
  async function toggleRaceFavourite(raceId: number) {
    setError("");
    ...
    try {
      ...
      if (existing && isFavouritedStatus(existing.status)) {
        const { error } = await supabase.from("user_races").delete().eq("id", existing.id);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {popularTop5.length > 0 ? (
            popularTop5.map((race, i) => {
              const userRace = userRaces.find((ur) => ur.race_id === race.race_id);
              const favourited = userRace ? isFavouritedStatus(userRace.status) : false;
              return (
                <div
                  key={race.race_id}
                ...
                >
                  ...
                  <div className="aspect-[4/3] relative bg-[var(--secondary)]/5 overflow-hidden">
                    <Link href={race.slug ? `/races/${race.slug}` : "#"} className="block h-full">
                      {race.hero_image_url ? (
                        <img
                    ...
                    <FavouriteHeartIconButton
                      overlay
                      active={favourited}
                      loading={busyRaceId === race.race_id}
                      label={
                        favourited ? "Remove race from your lists" : "Add race to Want"
                      }
                      onPress={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        void toggleRaceFavourite(race.race_id);
                      }}
                    />
                  </div>

                  <div className="p-4 border-t border-[var(--border)]">
                    <Link href={race.slug ? `/races/${race.slug}` : "#"}>
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {filteredCatalog.map((race) => {
              const userRace = userRaces.find((ur) => ur.race_id === race.id);
              const favourited = userRace ? isFavouritedStatus(userRace.status) : false;
              return (
                <div
                  key={race.id}
                  ...
                  <div className="aspect-[4/3] relative bg-[var(--secondary)]/5 overflow-hidden">
                    ...
                    <FavouriteHeartIconButton
                      ...
                      onPress={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        void toggleRaceFavourite(race.id);
                      }}
                    />
                  </div>
        ...
        <div className="space-y-10">
          <section className="space-y-4">
            <h2
              ...
              style={{ fontFamily: "var(--font-space-grotesk)" }}
            >
              Your Want To Go (Races)
            </h2>
            {userRaces.filter((u) => u.status === "want").length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {userRaces
                  .filter((ur) => ur.status === "want")
                  .map((ur) => {
                    const r = raceById(ur.race_id);
                    if (!r) return null;
                    const onList = isFavouritedStatus(ur.status);
                    return (
                      <div
                        key={ur.id}
                        className="group card overflow-hidden hover:shadow-lg transition-all relative"
                      >
                        <div className="aspect-[4/3] relative bg-[var(--secondary)]/5 overflow-hidden">
                          {r.slug ? (
                            <Link href={`/races/${r.slug}`} className="block h-full">
                              {r.hero_image_url ? (
                                <img
                          ...
                          <FavouriteHeartIconButton
                            overlay
                            active={onList}
                            loading={busyRaceId === ur.race_id}
                            label="Remove from bucket list"
                            onPress={(e) => {
                              e.preventDefault();
                              e.stopPropagation();
                              void toggleRaceFavourite(ur.race_id);
                            }}
                          />
                        </div>
                        <div className="p-4 border-t border-[var(--border)]">
              ...
              <p className="opacity-70 text-sm">No races in Want To Go yet.</p>
            )}
          </section>
          ...
          <section className="space-y-4">
            ...
            {userRaces.filter((u) => u.status === "been").length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {userRaces
                  .filter((ur) => ur.status === "been")
                  .map((ur) => {
                    const r = raceById(ur.race_id);
                    if (!r) return null;
                    const onList = isFavouritedStatus(ur.status);
              ...
              <p className="opacity-70 text-sm">No races marked as been yet.</p>
            )}
          </section>
        </div>
      )}
